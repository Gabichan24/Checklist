...
<div x-data="{ open: false }" class="max-w-xl mx-auto bg-white shadow rounded-lg p-6">
    ...
    <div class="flex items-center justify-between mb-4">
        ...
        <button @click="open = true" class="bg-purple-600 text-gray px-4 py-2 rounded-lg hover:bg-purple-700">+ Crear</button>
    </div>
            ...
            <tbody>
                @forelse($regiones as $region)
                    <tr class="text-center">
                        <td class="px-4 py-2 border">{{ $region->id }}</td>
                        <td class="px-4 py-2 border">{{ $region->nombre }}</td>
                        <td class="px-4 py-2 border">{{ $region->estados }}</td>
                        <td class="px-4 py-2 border">
                            <span class="bg-green-100 text-green-600 px-2 py-1 rounded-full text-xs">{{ $region->estatus }}</span>
                        </td>
        ...
        <div class="bg-white rounded-lg p-6 w-96 relative">
            ...
            <form action="{{ route('regiones.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="estados" class="w-full border rounded px-3 py-2" required>
    </select>
                </div>
